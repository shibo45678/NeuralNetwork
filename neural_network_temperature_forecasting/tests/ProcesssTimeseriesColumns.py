...
from  data.processing import ProcessTimeseriesColumns
...
def test_process_timeseries_columns():
    ...
    try:
        processor = ProcessTimeseriesColumns(col='string_date', format='%Y%m%d')
    ...
    try:
        ...
        print("✅ Unix时间戳处理成功")
        print("时间跨度特征:", result_timestamp[['days_since_start', 'year_since_start']].head())
    except Exception as e:
    ...
    df_seasons = pd.DataFrame({
        'date': pd.date_range('2023-01-01', periods=12, freq='M'),  # 全年各月
        'value': range(12)
    })
    ...
    try:
        ...
        # 检查季节划分是否正确
        season_mapping = result_seasons.groupby('season')['date'].count()
