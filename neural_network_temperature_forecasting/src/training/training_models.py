...
def TrainingCnn(model,# tf.keras.models
                ...
                verbose:int=2,
                file_path:str='best_model_cnn.keras'):
    ...
    history_plot(history = record, model_name="CNN Model")

    """加载最佳模型进行预测或继续训练"""
    best_model_cnn = tf.keras.models.load_model(file_path)

    return record,best_model_cnn
...
def TrainingLstm(model:tf.keras.models,
                 ...
                 epochs:int=100,
                 verbose:int=2,
                 file_path:str ='best_model_lstm.keras'):

    record = model.fit(window.createTrainSet, # 或者直接写window 训练数据也可以不写
                        validation_data=window.createValSet,
                        epochs=epochs,
                        verbose=verbose,
                        callbacks=[
                            ...
                            tf.keras.callbacks.ReduceLROnPlateau(
                                ...
                                min_lr=1e-6,  # 最小学习率。学习率降低就是让模型"慢下来，仔细找
                                verbose=2
                            )
                        ]
                        )
    """训练过程可视化"""
    # record.history 为字典对象，包含训练过程中的loss的测量指标等记录项
    history_plot(history=record, model_name="LSTM Model")

    # 加载最佳模型进行预测或继续训练
    best_model_lstm = tf.keras.models.load_model(file_path)

    return record,best_model_lstm
