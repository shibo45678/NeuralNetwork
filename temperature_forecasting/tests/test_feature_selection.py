...
import pytest
from data.feature_engineering.feature_selection import BasedOnCorrSelector
...
def test_normal_training_mode(sample_data):
    ...
    selector = BasedOnCorrSelector(threshold=0, mode='train')
...
def test_high_threshold(sample_data):
    ...
    X_train, y_train = sample_data
    print("\n=== 测试案例3：高阈值情况 ===")
    selector_high_threshold = BasedOnCorrSelector(threshold=0.9, mode='train')
...
def test_no_target_variable(sample_data):
    ...
    X_train, y_train = sample_data
    print("\n=== 测试案例4：无目标变量情况 ===")
    selector_no_y = BasedOnCorrSelector(threshold=0.1, mode='predict')
